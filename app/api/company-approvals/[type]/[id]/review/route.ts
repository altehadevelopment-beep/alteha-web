...
// POST /api/company-approvals/{type}/{id}/review — aprobar/rechazar registro de empresa.
export async function POST(request: NextRequest, { params }: { params: { type: string; id: string } }) {
    try {
        const userToken = request.headers.get('X-Alteha-Token');
        const adminToken = await getAppToken();
        const body = await request.text();
        const response = await fetch(`${API_BASE}/company-approvals/${params.type}/${params.id}/review`, {
            method: 'POST',
            headers: {
