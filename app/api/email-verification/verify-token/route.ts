...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_BASE}/email-verification/verify-token`, {
            method: 'POST',
            headers: {
                'Accept': '*/*',
        ...
        if (!response.ok) {
            ...
            console.error(`Email verification check failed: ${response.status} ${response.statusText}`, text);
            try {
