...
// POST — el seguro cancela su subasta (reglas por estado + motivo).
export async function POST(request: NextRequest, { params }: { params: { auctionNumber: string } }) {
    try {
        ...
        const response = await fetch(`${API_BASE}/cancellations/auction/${encodeURIComponent(params.auctionNumber)}`, {
            method: 'POST',
            headers: {
