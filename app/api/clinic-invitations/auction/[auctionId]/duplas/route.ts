...
// GET /api/clinic-invitations/auction/[auctionId]/duplas — insurer view of doctor+clinic duplas.
export async function GET(request: NextRequest, { params }: { params: { auctionId: string } }) {
    try {
        ...
        const response = await fetch(`${API_BASE}/clinic-invitations/auction/${params.auctionId}/duplas`, {
            method: 'GET',
            headers: {
        ...
        return NextResponse.json(Array.isArray(data) ? data : (data?.content ?? data?.data ?? []), { status: 200 });
    } catch (error: any) {
