...
// Proxy de sub-rutas de exclusión: /api/exclusion-lists/* → backend /api/exclusion-lists/*
async function forward(request: NextRequest, params: { path: string[] }, method: string) {
    try {
        const userToken = request.headers.get('X-Alteha-Token');
        const adminToken = await getAppToken();
        const url = new URL(request.url);
        const target = `${API_BASE}/exclusion-lists/${params.path.join('/')}${url.search}`;
...
export async function GET(request: NextRequest, { params }: { params: { path: string[] } }) { return forward(request, params, 'GET'); }
export async function POST(request: NextRequest, { params }: { params: { path: string[] } }) { return forward(request, params, 'POST'); }
