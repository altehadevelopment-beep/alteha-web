...
// Proxy de órdenes de despacho: /api/dispatch-orders/* → backend /api/dispatch-orders/*
async function forward(request: NextRequest, params: { path: string[] }, method: string) {
    try {
        const userToken = request.headers.get('X-Alteha-Token');
        const adminToken = await getAppToken();
        const url = new URL(request.url);
        const target = `${API_BASE}/dispatch-orders/${params.path.join('/')}${url.search}`;
        ...
        console.log(`[API Proxy] ${method} /dispatch-orders/${params.path.join('/')} - Status: ${response.status}`);
