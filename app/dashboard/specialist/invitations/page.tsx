...
import {
    Mail, Building2, DollarSign, CheckCircle2, XCircle, Clock,
    Loader2, AlertTriangle, CreditCard, Gavel, Inbox, TrendingDown
} from 'lucide-react';
import { getDoctorDuplaInvitations, respondDoctorDuplaInvitation, getPaymentMethods } from '@/lib/api';
...
/**
 * Bandeja del médico: invitaciones de clínicas que ofertaron "Solo Clínica" y lo
 * eligieron para la dupla. Al aceptar define sus honorarios — puede colocarlos por
 * debajo del presupuesto médico de la subasta, bajando el costo total de la oferta.
 */
export default function DoctorInvitationsPage() {
    const [invitations, setInvitations] = useState<any[]>([]);
    const [doctorMethods, setDoctorMethods] = useState<string[]>([]);
    ...
    const load = async () => {
        try {
            const [invs, methods] = await Promise.all([
                getDoctorDuplaInvitations().catch(() => []),
    ...
    const accept = async (inv: any) => {
        ...
        if (inv.doctorBudget != null && fee > Number(inv.doctorBudget)) {
            setError(`Tus honorarios no pueden superar el presupuesto médico de la subasta ($${money(inv.doctorBudget)}). Sí puedes ofrecerlos por debajo.`);
            return;
        }
        setBusyId(inv.id);
        try {
    ...
    const reject = async (inv: any) => {
        ...
        setBusyId(inv.id);
        try {
            const res = await respondDoctorDuplaInvitation(inv.id, { accepted: false });
            if (res?.id || res?.data?.id || res?.status === 'REJECTED' || res?.code === '00') {
                await load();
            } else {
        ...
        <div className="space-y-8 font-outfit max-w-4xl mx-auto pb-20">
            <header className="space-y-1">
                ...
                <p className="text-slate-400 text-sm font-medium">
                    Invitaciones de clínicas para participar en subastas. Si aceptas, defines tus honorarios —
                    puedes colocarlos por debajo del presupuesto médico para hacer la oferta más competitiva — y se arma la dupla visible para el seguro.
                </p>
            </header>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-16 text-center space-y-3">
                    ...
                    <p className="text-sm text-slate-400">Cuando una clínica te invite a una subasta, aparecerá aquí.</p>
                </div>
            ) : (
                    ...
                    {pending.length > 0 && (
                        <section className="space-y-4">
                            ...
                            {pending.map((inv) => {
                                ...
                                const fee = parseFloat(feeInputs[inv.id] ?? '');
                                const totalPreview = !isNaN(fee) ? fee + Number(inv.clinicFee || 0) : null;
                                return (
                                    <div key={inv.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 space-y-5">
                                        <div className="flex items-start justify-between gap-4 flex-wrap">
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-sm text-slate-500 font-medium flex items-center gap-1.5">
                                                    ...
                                                    Te invitó: <span className="font-bold text-slate-700">{inv.clinicName || 'Clínica'}</span>
                                                </p>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Oferta de la clínica</p>
                                                <p className="text-xl font-black text-slate-900">${money(inv.clinicFee)}</p>
                                            </div>
                                            <div className="bg-slate-50 rounded-2xl p-4">
                                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tu tope (presupuesto médico)</p>
                                                <p className="text-xl font-black text-slate-900">{inv.doctorBudget != null ? `$${money(inv.doctorBudget)}` : 'Sin tope'}</p>
                                            </div>
                                        </div>

                                        {/* Compatibilidad de métodos de cobro con la subasta */}
                                        {!compat.ok ? (
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">
                                                    Tus honorarios (USD) — puedes bajarlos para reducir el total de la oferta
                                                </label>
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        type="number"
                                                        min={0}
                                                        max={inv.doctorBudget != null ? Number(inv.doctorBudget) : undefined}
                                                        value={feeInputs[inv.id] ?? ''}
                                                        onChange={(e) => setFeeInputs((p) => ({ ...p, [inv.id]: e.target.value }))}
                                                        placeholder={inv.doctorBudget != null ? `Máximo ${money(inv.doctorBudget)}` : '0.00'}
                                                ...
                                                {totalPreview != null && (
                                                    <p className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
                                                        ...
                                                        Total de la dupla para el seguro:
                                                        <span className="font-black text-slate-800">${money(totalPreview)}</span>
                                                        {inv.doctorBudget != null && fee < Number(inv.doctorBudget) && (
                                                            <span className="text-emerald-600 font-bold">
                                                                (ahorras ${money(Number(inv.doctorBudget) - fee)} al seguro)
                                                            </span>
                                                        )}
                                                    </p>
                                                )}
                                ...
                                return (
                                    <div key={inv.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-5 flex items-center justify-between gap-4 flex-wrap">
                                        <div className="space-y-0.5">
                                            ...
                                            <p className="text-xs text-slate-500 font-medium">{inv.clinicName} · Oferta clínica ${money(inv.clinicFee)}</p>
                                        </div>
                                            ...
                                            {accepted ? (
                                                <>
                                                    ...
                                                    <p className="text-xs text-slate-500 font-medium mt-1">
                                                        Tus honorarios: <span className="font-black text-slate-800">${money(inv.doctorFee)}</span> · Total dupla: <span className="font-black text-slate-800">${money(inv.total)}</span>
                                                    </p>
                                                </>
                                            ) : (
