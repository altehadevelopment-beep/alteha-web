...
export default function ClinicInvitationsPage() {
    ...
    const accept = async (inv: any) => {
        ...
        try {
            ...
            const res = await respondClinicInvitation(inv.id, { accepted: true, clinicFee: fee, paymentMethodId: chosen ? Number(chosen) : undefined });
                            ...
                            {pending.map((inv) => {
                                ...
                                return (
                                    <div key={inv.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6 space-y-5">
                                        ...
                                        {/* Método de cobro para la liquidación de tu parte */}
                                        {compat.ok && myMethods.length > 0 && (
                                            <div className="space-y-1.5">
                                                ...
                                                <p className="text-[10px] text-slate-400 font-medium">
                                                    Alteha liquidará tu parte a este método cuando se complete la subasta.
                                                </p>
                                            </div>
                                        )}
