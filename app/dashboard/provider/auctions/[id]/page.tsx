...
export default function ProviderAuctionDetailPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto font-outfit pb-20 px-4 md:px-0">
            <Link href="/dashboard/provider/auctions" className="flex items-center gap-2 text-slate-400 hover:text-alteha-violet transition-colors mb-8 font-bold group w-fit">
                ...
                <div className="bg-slate-900 p-10 text-white relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                            ...
                            #{auction.auctionNumber} • Requerimientos de Insumos
                        </p>
                    </div>
                </div>

                {/* Summary stats */}
                <div className="grid grid-cols-2 md:grid-cols-3 divide-x divide-slate-100 uppercase tracking-widest">
                    <div className="p-6 text-center">
                        <p className="text-[10px] font-black text-slate-400 mb-1">Items Requeridos</p>
                        <p className="text-xl font-black text-slate-900">{auction.requiredSupplies?.length || 0}</p>
                    </div>
                    <div className="p-6 text-center">
                    ...
                    <div className="p-6 text-center">
                        <p className="text-[10px] font-black text-slate-400 mb-1">Especialidad</p>
                        <p className="text-sm font-black text-slate-900 truncate px-2">{auction.specialty?.name || 'Varios'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-8">
                ...
                {(auction.status === 'ACTIVE' || auction.status === 'PUBLISHED') && (
                    <div className="space-y-4">
                        <h2 className="text-2xl font-black text-slate-900 italic">Cotizar Suministros</h2>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-100 border border-slate-50 p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {auction.requiredSupplies?.map((s, idx) => (
                            <div key={idx} className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className="font-black text-slate-900">{s.itemName}</h4>
                                    <span className="px-3 py-1 bg-white rounded-lg text-xs font-black text-alteha-violet border border-slate-100">
                                        Cant: {s.quantity}
                                    </span>
                                </div>
